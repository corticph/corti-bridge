...
- The harness never replays `server_tool_use` / `advisor_tool_result` blocks into the next request's history — verified against the raw client bodies in the debug logs. In `openai` hold-and-continue mode the advisor's text therefore lives only in the UI and in whatever the continuation says in its own words. That is why `endContinuationFailure` repeats the advice verbatim instead of pointing at it: a note saying "proceed using the advice above" points at nothing, and the model answers the next turn from a context where the consult left no trace.
