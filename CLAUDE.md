...
`test/dispatch.sh` runs lifecycle tests against a *copy* of the gateway with the Corti-URL check relaxed; never point `CC_PROXY_DIR` at a live clone when running it (its pattern-kill fallback matches by clone path).
...
- Runtime state lives in `~/.corti-claude/` (`models.env`, `profile.env`, pid files, `gateway.log`); the repo itself stays stateless.
...
- `test/models.sh` has 2 pre-existing failures (model-tier selection) as of 2026-08-31 — don't chase them as regressions of unrelated work.
