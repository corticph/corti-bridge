  ...
  for (const b of blocks) {
    ...
    switch (b.type) {
      ...
      case "advisor_tool_result": {
        // The advisor's own prior guidance, replayed in the transcript. The advisor needs to see
        // what it said before to reconcile (its prompt: "surface the conflict in one more advisor
        // call") — without this it can't tell the executor which prior advice to build on or
        // correct. The content is a discriminated union; render the advice text (success) or an
        // unavailable note (error), head/tail-truncated like any large tool result.
        const c = b.content;
        let advice = "";
        if (c && typeof c === "object") {
          if (c.type === "advisor_result" && typeof c.text === "string") advice = c.text;
          else if (c.type === "advisor_tool_result_error") advice = `advisor unavailable (${c.error_code || "unavailable"})`;
        }
        if (advice) lines.push(`← advisor guidance: ${truncateMiddle(advice, maxToolResultChars)}`);
        else lines.push("[advisor_tool_result]");
        break;
      }
      default:
