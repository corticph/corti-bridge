...
// Implements .context/projects/switch-to-openai-endpoints/plan.md steps 3–4.
...
export function translateRequest(body) {
  ...
  const req = {
    model: body.model,
    messages,
    // CC >= 2.1 omits max_tokens; 32000 matches its classic default
    max_tokens: 32000,
  };
  ...
  const droppedTools = (Array.isArray(body.tools) ? body.tools : []).length - tools.length;
...
export function translateError({ status, headers = {}, bodyText = "", requestedModel = "" }) {
  ...
  if (status === 400 && OVERFLOW_RE.test(detail)) {
    ...
    const n = /request has (\d+) input tokens/.exec(detail)?.[1];
    const message =
      max && n
        ? `prompt is too long: ${n} tokens > ${max} maximum (upstream: ${detail})`
        : `prompt is too long (upstream: ${detail})`;
